...
const generateStartingTerrain = () => {
  for (let i = 0; i < krat; i++) {
    for (let j = 0; j < krat; j++) {
      const x = simplex.noise2D(i / 12, j / 12)
      // const lasy = simplex.noise2D(i / 12, j / 12)
      // x*2+1  ->  1 - 3
      if (x > 0) {
        if (x * 2 + 1 >= 1 && x * 2 + 1 < 2) {
...
let tick = 0
let lastTick = 0
const focus = { x: Math.floor(krat / 2), y: Math.floor(krat / 2) }
const cursor = { x: Math.floor(krat / 2), y: Math.floor(krat / 2) }
const d = [1, 1]
...
let loop = () => {
  ...
  const offsetH = _H / 2 - a * hexcoords(focus.x, focus.y).y - a * 3 / 4
  const offsetW = _W / 2 - a * hexcoords(focus.x, focus.y).x - a
